...
static int qmi_wwan_bind_gobi(struct usbnet *dev, struct usb_interface *intf)
{
    ...
    if (dev->udev->descriptor.bcdDevice == cpu_to_le16(0x232)  && intf->cur_altsetting->desc.bInterfaceNumber == 5)
    {
        printk("sim7100 device\n");
        //dev_err(&intf->dev, "dev:Sim7100 device!\n");
    }
    else
    {
      rv = -ENODEV;
