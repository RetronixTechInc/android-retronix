...
int _IOGetPhyMem(int which, vpu_mem_desc *buff)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
    ret = ion_query_heap_cnt(fd, &heap_cnt);
    if (ret != 0 || heap_cnt == 0) {
        err_msg("can't query heap count");
        return -1;
    }

    memset(&ihd, 0, sizeof(ihd));
    ret = ion_query_get_heaps(fd, heap_cnt, &ihd);
    if (ret != 0) {
        err_msg("can't get ion heaps");
        return -1;
    }
    heap_mask = 0;
    ...
    err = ion_alloc_fd(fd, buff->size, pagesize, heap_mask, ION_FLAG_CACHED, &share_fd);
	if (err) {
		err_msg("ion allocation failed!\n");
		return -1;
	}

    err = ioctl(share_fd, DMA_BUF_IOCTL_PHYS, &dma_phys);
	if (err) {
		err_msg("ioctl phys failed!\n");
        close(share_fd);
		goto error;
	}
    err = dma_phys.phys;

    ptr = (unsigned char*)mmap(0, buff->size, PROT_READ|PROT_WRITE,
	...
	err = ion_alloc(fd, buff->size, pagesize, 1, ION_FLAG_CACHED, &handle);
#else
	...
	err = ion_phys(fd, handle, share_fd);
	if (err == 0) {
...
#else
	va_addr = (unsigned long)mmap64(NULL, size, PROT_READ | PROT_WRITE,
                       MAP_SHARED, vpu_fd, buff.cpu_addr);
#endif
