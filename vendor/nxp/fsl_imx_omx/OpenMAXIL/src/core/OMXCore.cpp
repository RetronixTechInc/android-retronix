...
OMX_ERRORTYPE OMXCore::OMX_Init()
{
	...
	ret = ComponentRegister(ComponentRegisterFile);
...
OMX_ERRORTYPE OMXCore::ComponentRegister(OMX_STRING filename)
{
	OMX_ERRORTYPE ret = OMX_ErrorNone;
	...
	ret_reg = RegistryAnalyser.Open(filename);
	if (ret_reg != REG_SUCCESS)
	{
		LOG_ERROR("Open component register file %s fail.\n", filename);
	...
	do
	{
		...
		for (EntryIndex=0; ;EntryIndex++)
		{
			...
			else
			{
                ...
                else if (!fsl_osal_strcmp(pRegEntryItem->name, "include_file"))
				{
                    // ignore error when parsing included component register
					ComponentRegister(pRegEntryItem->value);
				}
				else
				{
