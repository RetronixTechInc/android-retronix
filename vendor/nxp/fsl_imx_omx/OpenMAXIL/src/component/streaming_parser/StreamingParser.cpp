        ...
        uint32 stream_id)
{
    ...
    pPortDef->format.video.nFrameHeight = ffcodec->height;
...
OMX_S32 StreamingParser::UrlHeaderPos(OMX_STRING url)
{
...
OMX_ERRORTYPE StreamingParser::InitStreaming()
{
    ...
    OMX_S32 pos = UrlHeaderPos((OMX_STRING)pMediaName);
    if(pos > 0) {
        OMX_S32 url_len = fsl_osal_strlen((const fsl_osal_char*)pMediaName) + 1;
        URL = (OMX_STRING)FSL_MALLOC(url_len);
        if(URL == NULL)
            return OMX_ErrorInsufficientResources;
        fsl_osal_strcpy(URL, (const fsl_osal_char*)pMediaName);
        URL[pos - 1] = '\0';
        mURL = URL;
        mHeader = URL + pos;
    }
    else
        mURL = (OMX_STRING)pMediaName;

    LOG_DEBUG("mURL: %s, mHeader: %s\n", mURL, mHeader);
    fsl_osal_strcpy(ic->filename, (char*)mURL);
    if (mHeader)
        fsl_osal_strcpy(ic->headers, (char*)mHeader);
    ...
    int retry_count = 0, max_retry_count = 10;
    ...
    while(1){
        ...
        printf("read_header fail %d, retry %d\n", err, retry_count);
    }

    SendEvent(OMX_EventBufferingDone, 0, 0, NULL);
    if (err < 0) {
        printf("read_header totally fail!!!\n");
...
OMX_ERRORTYPE StreamingParser::ReadPacketFromStream()
{
    ...
    if(bSeek){
        OMX_TICKS seekPos = (bVideoActived ? sVideoSeekPos:sAudioSeekPos);
        //drop the package after seek
        if(pkt->pts > seekPos + PACKET_CACHE_SIZE){
            LOG_ERROR("drop one frame,ts=%lld,seekpos=%lld",pkt->pts,seekPos);
            FREE_PKT(pkt);
            fsl_osal_mutex_unlock(Lock);
            fsl_osal_sleep(0);
            return OMX_ErrorNone;
