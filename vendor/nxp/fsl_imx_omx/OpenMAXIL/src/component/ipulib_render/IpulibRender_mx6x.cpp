...
#define SURFACE_REGION_CHECK_PERIOD    100000    // ipu render will out of sync if the check period is larger than 100ms
...
static OMX_S32 ConvertRotationDevice2Ipu(OMX_S32 value)
{
    OMX_S32 result;
    ...
    switch(value) {
        case ROT_0:
            ...
            break;
        case ROT_90:
            result = ROTATE_90_RIGHT;
            break;
        case ROT_180:
            result = ROTATE_180;
            break;
        case ROT_270:
            result = ROTATE_90_LEFT;
            break;
        case FLIP_H:
            result = ROTATE_HORIZ_FLIP;
            break;
        case FLIP_V:
            result = ROTATE_VERT_FLIP;
            break;
        default:
            result = ROTATE_NONE;
    }
...
void *ConfigScreenThreadFunc(fsl_osal_ptr arg)
{
    IpulibRender *pRender = (IpulibRender*)arg;
    OMX_BOOL doTrans;

    while(1) {
        if (pRender->bStopThread) {
            break;
        }

        pRender->CheckScreenConfig(&doTrans);
        if (doTrans) {
            OMX_STATETYPE state = OMX_StateInvalid;
            pRender->GetState(&state);
            if (state == OMX_StatePause)
                pRender->RenderFrame((OMX_PTR)pRender->mTask.input.paddr);
        }

        fsl_osal_sleep(SURFACE_REGION_CHECK_PERIOD);
    }
...
{
    if(E_FSL_OSAL_SUCCESS != fsl_osal_thread_create(&pThread, NULL, ConfigScreenThreadFunc, this)) {
        return OMX_ErrorInsufficientResources;
    }
    ...
    while(ret != IPU_CHECK_OK && ret > IPU_CHECK_ERR_MIN) {
        ...
        switch(ret) {
            ...
            case IPU_CHECK_ERR_SPLIT_OUTPUTH_OVER:
                ...
                break;
            default:
                if (ret != IPU_CHECK_ERR_SPLIT_WITH_ROT)
...
OMX_ERRORTYPE IpulibRender::FBInit()
{
    ...
    OMX_S32 nBufferSize = fscreeninfo.line_length * screen_info.yres_virtual / MAX_FB_BUFFERS;
...
OMX_ERRORTYPE IpulibRender::SetFBAlpha(OMX_U32 alpha)
{
    struct mxcfb_gbl_alpha gbl_alpha;
    struct mxcfb_color_key key;
    int fb0;

    fb0 = open("/dev/graphics/fb0", O_RDWR | O_NONBLOCK, 0);
    if(fb0 < 0) {
        LOG_ERROR("Open device: /dev/graphics/fb0 failed.\n");
        return OMX_ErrorNone;
    }
    gbl_alpha.alpha  = alpha;    //  0~255
    gbl_alpha.enable = 1;
    int ret = ioctl(fb0, MXCFB_SET_GBL_ALPHA, &gbl_alpha);
    if(ret < 0) {
        LOG_ERROR("Error! MXCFB_SET_GBL_ALPHA failed!\n");
        close(fb0);
        return OMX_ErrorUndefined;
    }
    ...
    ret = ioctl(fb0, MXCFB_SET_CLR_KEY, &key);
    if(ret < 0) {
        LOG_ERROR("Error! Colorkey setting failed for dev!\n");
        close(fb0);
        return OMX_ErrorUndefined;
    }
...
OMX_ERRORTYPE IpulibRender::CheckScreenConfig(OMX_BOOL *bTrans)
{
    int fd;
    int* buf = NULL;
    SURFACE_REGION* pRegionTmp = NULL;

    *bTrans = OMX_FALSE;

    if (hSidebandHandle)
        fd = hSidebandHandle->data[0];
    else
        return OMX_ErrorNone;

    buf = (int*)mmap(0, sizeof(SURFACE_REGION), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if ((void*)buf == (void*)-1)
        return OMX_ErrorUndefined;

    pRegionTmp = (SURFACE_REGION *)buf;

    if (pRegionTmp->state == TRANSCATION_STATE_START) {
        *bTrans = OMX_TRUE;
    }
    ...
    if (*bTrans) {
        ...
        mTask.output.rotate = ConvertRotationDevice2Ipu(pRegionTmp->rotation);
        mTask.output.crop.pos.x = (pRegionTmp->left + 7)/8*8;
        mTask.output.crop.pos.y = pRegionTmp->top;
        mTask.output.crop.w = (pRegionTmp->right - pRegionTmp->left)/8*8;
        mTask.output.crop.h = pRegionTmp->bottom - pRegionTmp->top;

        pRegionTmp->state = TRANSCATION_STATE_NONE;
        LOG_DEBUG("seting rotate %d l %d t %d r %d b %d \n", \
            pRegionTmp->rotation, pRegionTmp->left, pRegionTmp->top, pRegionTmp->right, pRegionTmp->bottom);

        fsl_osal_mutex_unlock(lock);
    }
    munmap(buf, sizeof(SURFACE_REGION));
    return OMX_ErrorNone;
}
