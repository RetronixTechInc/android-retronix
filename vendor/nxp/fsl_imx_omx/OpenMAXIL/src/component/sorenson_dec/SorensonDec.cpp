...
SorensonDec::SorensonDec()
{
    ...
    sInFmt.eCompressionFormat =(OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingSORENSON263;
...
OMX_ERRORTYPE SorensonDec::InitFilter()
{
    ...
    LOG_DEBUG("sInFmt %d/%d, decoded width %d,height %d, luma stride %d\n",sInFmt.nFrameWidth,sInFmt.nFrameHeight, Info.width,Info.height,Info.stride);
...
void SorensonDec::CopyOutputFrame(OMX_U8 *pOutputFrame)
{
        ...
        if(sOutFmt.eColorFormat != OMX_COLOR_Format16bitRGB565){
            ...
            // use stride and aligned height, instead of Info.width/height,
            // don't consider cropping when doing color space convert.
            // cropping will be done in render, by sOutCrop passed from decoder to render
            OMX_IMAGE_PORTDEFINITIONTYPE in_fmt, out_fmt;
            in_fmt.nFrameWidth = Info.stride;//Info.width;
            in_fmt.nFrameHeight = aligned_height;//Info.height;
            in_fmt.nStride = Info.stride;
            in_fmt.eColorFormat = OMX_COLOR_FormatYUV420Planar;
            out_fmt.nFrameWidth = nPadWidth;
            out_fmt.nFrameHeight = nPadHeight;
            out_fmt.nStride = nPadWidth;
            out_fmt.eColorFormat = sOutFmt.eColorFormat;

            OMX_CONFIG_RECTTYPE crop;
            crop.nLeft = crop.nTop = 0;
            crop.nWidth = nPadWidth;
            crop.nHeight = nPadHeight;

            OMX_S32 ret = pImageConvert->resize(pImageConvert, &in_fmt, &crop,
                                    pOutputFrame, NULL, NULL,
