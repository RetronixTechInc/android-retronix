...
AUDIO_FILTERRETURNTYPE Mp3Dec::AudioFilterFrame()
{
	...
	MP3D_RET_TYPE Mp3Ret;
	Mp3Ret = mp3d_decode_frame(pMp3DecConfig, pMp3DecParams, \
            (MP3D_INT32 *)pOutBufferHdr->pBuffer);

	if (Mp3Ret == MP3D_END_OF_STREAM)
		pMp3DecConfig->consumedBufLen = nActuralLen;
	LOG_DEBUG("Audio stream frame len = %d, mp3 ret %d\n", \
            pMp3DecConfig->consumedBufLen, Mp3Ret);
	AudioRingBuffer.BufferConsumered(pMp3DecConfig->consumedBufLen);

    // Work around for MP3 layer 3 decoder will consume all bitstream for first granue decode.
    // The second granue will consume 0 bytes.
    if (pMp3DecParams->layer == 3) {
        TS_Manager.Consumered(AudioRingBuffer.nPrevOffset);
        AudioRingBuffer.nPrevOffset = pMp3DecConfig->consumedBufLen;
    }
	...
	if (Mp3Ret == MP3D_OK || Mp3Ret == MP3D_ERROR_SCALEFACTOR)
	{
		...
		LOG_DEBUG("nSampleRate: %d nChannels: %d\n", pMp3DecParams->mp3d_sampling_freq, \
                pMp3DecParams->mp3d_num_channels);
        TS_Manager.SetOneByteTime(OMX_TICKS_PER_SECOND/(Mp3Type.nBitRate>>3));
				...
				|| Mp3Type.nSampleRate != (OMX_U32)pMp3DecParams->mp3d_sampling_freq)
		{
			...
			LOG_DEBUG("Decoder Sample Rate = %d\t TS per Frame = %lld\n", \
                    Mp3Type.nSampleRate, TS_PerFrame);
