        ...
        TRACK_DATA *pData)
{
    ...
    err = hItf.AddSample(hMuxer, trackId, (uint8*)pData->pBuffer, pData->nFilledLen,
            -1, pData->nTimeStamp, pData->nDuration, flags);
	LOG_DEBUG("AddSample ret: %d.\n", err);
	if(err == MUXER_WRITE_ERROR) {
		return OMX_ErrorInsufficientResources;
	}
    else if(err == MUXER_ERR_INVALID_PARAMETER){
        return OMX_ErrorBadParameter;
    }
...
OMX_ERRORTYPE FslMuxer::AddTrackRotate(OMX_U32 track, OMX_U32 rotateDegree)
{
    hItf.SetTrackProperty(hMuxer, track, PROPERTY_FRAME_ROTATE_DEGREE, DATA_FORMAT_UINT32,
            &rotateDegree, sizeof(OMX_U32));
    return OMX_ErrorNone;
}
