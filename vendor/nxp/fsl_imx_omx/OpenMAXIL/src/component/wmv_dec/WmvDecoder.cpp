...
OMX_ERRORTYPE WmvDecoder::GetOutputBuffer(OMX_PTR *ppBuffer,OMX_S32* pOutSize)
{
    ...
    if(sOutFmt.eColorFormat != OMX_COLOR_Format16bitRGB565){
        ...
        in_fmt.nFrameWidth = sDecObj.sDecParam.sOutputBuffer.s32YRowSize;
        in_fmt.nFrameHeight = nPadHeight;
        in_fmt.nStride = sDecObj.sDecParam.sOutputBuffer.s32YRowSize;
        in_fmt.eColorFormat = OMX_COLOR_FormatYUV420Planar;
        out_fmt.nFrameWidth = sDecObj.sDecParam.sOutputBuffer.s32YRowSize;
        out_fmt.nFrameHeight = nPadHeight;
        out_fmt.nStride = sOutFmt.nFrameWidth;
        out_fmt.eColorFormat = sOutFmt.eColorFormat;
        //printf("row size %d, Frame w/h %d/%d, pad height %d",
        //    sDecObj.sDecParam.sOutputBuffer.s32YRowSize, sOutFmt.nFrameWidth, sOutFmt.nFrameHeight, nPadHeight);
