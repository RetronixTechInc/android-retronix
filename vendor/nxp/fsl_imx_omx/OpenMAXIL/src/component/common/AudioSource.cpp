...
#define AUDIO_DATA_LOST_THRESHOLD 3000000
...
OMX_ERRORTYPE AudioSource::SendOutputBuffer()
{
    ...
    if(sCur.nTimestamp - MediaTime > AUDIO_DATA_LOST_THRESHOLD)
    {
        printf("adjust audio timestamp, clock time - audio ts: %lld", sCur.nTimestamp - MediaTime);
        TotalRecordedLen += (sCur.nTimestamp - MediaTime) * PcmMode.nSamplingRate * nSampleSize / OMX_TICKS_PER_SECOND;
    }
