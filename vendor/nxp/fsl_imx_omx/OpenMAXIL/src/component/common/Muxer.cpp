        ...
        void *context)
{
    ...
    return hMuxer->OpenContent(CP_AccessAsyncWrite);
}
...
OMX_ERRORTYPE Muxer::InstanceDeInit()
{
    ...
    for(i=0; i<nPorts; i++) {
        ...
        if(Tracks[i].bTrackAdded == OMX_TRUE && Tracks[i].bSampleAdded == OMX_FALSE){
            LOG_DEBUG("Muxer InstanceDeInit, Track %d no sample added, total port %d", i, nPorts);
            SendEvent(OMX_EventError, OMX_ErrorMalFormat, 0, NULL);
        }
        ...
        if(pData != NULL) {
          pData->nDuration = Tracks[i].nSampleDuration;
			...
			if (bAddSampleDone == OMX_FALSE) {
                OMX_ERRORTYPE err = AddOneSample(i, pData);
                ...
                if(OMX_ErrorBadParameter != err){
                    if(Tracks[i].bSampleAdded == OMX_FALSE)
                       Tracks[i].bSampleAdded = OMX_TRUE;
                }
...
OMX_ERRORTYPE Muxer::TakeOneBufferFromPort()
{
    ...
    for(i=0; i<nPorts; i++) {
        ...
        if(nReadyBuffers > 0 || nGetCodecDataMask != 0) {
            if(pBufferHdr[i] != NULL)
                AddToReadyBuffers(i);
