...
#if (ANDROID_VERSION <= ICS)
    bStoreMetadataInBuffer = OMX_FALSE;
...
OMX_ERRORTYPE GMRecorder::SetMuxerParameter()
{
	...
	OMX_SetParameter(Muxer->hComponent, OMX_IndexParamFileOffset64Bits, &gm_use64Bit);
...
OMX_ERRORTYPE GMRecorder::SetupPipeline()
{
	...
	if (bHasVideo == OMX_TRUE) {
        ...
        sStoreMetaData.bEnabled = bStoreMetadataInBuffer;
        ret = OMX_SetParameter(VideoSource->hComponent, OMX_IndexParamStoreMetaDataInBuffers, &sStoreMetaData);
        ret = OMX_SetParameter(VideoEncoder->hComponent, OMX_IndexParamStoreMetaDataInBuffers, &sStoreMetaData);

        if (bStoreMetadataInBuffer == OMX_TRUE) {
            OMX_PARAM_PORTDEFINITIONTYPE sPortDef;

            OMX_INIT_STRUCT(&sPortDef, OMX_PARAM_PORTDEFINITIONTYPE);
            sPortDef.nPortIndex = 1;
            OMX_GetParameter(VideoSource->hComponent, OMX_IndexParamPortDefinition, &sPortDef);
            sPortDef.nBufferSize = sizeof(METADATA_BUFFER);
            OMX_SetParameter(VideoSource->hComponent, OMX_IndexParamPortDefinition, &sPortDef);
...
OMX_ERRORTYPE GMRecorder::GetContentPipe(CP_PIPETYPE **Pipe)
{
    ...
    *Pipe = NULL;
    if(fsl_osal_strstr((fsl_osal_char*)(gm_path), "sharedfd://"))
        ret = OMX_GetContentPipe((void**)Pipe, (OMX_STRING)"ASYNC_WRITE_PIPE");
...
OMX_ERRORTYPE GMRecorder::stop()
{
    ...
    if(bMalFormat == OMX_TRUE)
        ret = OMX_ErrorMalFormat;
...
OMX_ERRORTYPE GMRecorder::setClient(const OMX_U16 * clientName, OMX_S32 clientUID)
{
    gm_clientName = clientName;
    gm_clientUID = clientUID;
    return OMX_ErrorNone;
}
