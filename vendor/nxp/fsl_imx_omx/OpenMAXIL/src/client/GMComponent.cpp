        ...
        OMX_STRING role)
{
    ...
    bIsSoftwareVideoDecoder = OMX_FALSE;
    for(; i<NumComps; i++) {
        ...
        if(ret == OMX_ErrorNone) {
            ...
            if(fsl_osal_strstr((fsl_osal_char*)name, "video_decoder")
                    && fsl_osal_strstr((fsl_osal_char*)name, "sw-based")) {
                bIsSoftwareVideoDecoder = OMX_TRUE;
            }
            break;
        }
    }
...
OMX_BOOL GMComponent::IsSoftwareBasedVideoDecoder()
{
	return bIsSoftwareVideoDecoder;
}
...
OMX_ERRORTYPE GMComponent::RegistEmptyDoneCallback(
        EMPTYDONECALLBACKTYPE emptyDoneCb,
        OMX_PTR pAppData)
{
    EmptyDoneCb = emptyDoneCb;
    EmptyDoneCbAppData = pAppData;
        ...
        OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if (EmptyDoneCb != NULL) {
        return (*EmptyDoneCb)(this, EmptyDoneCbAppData, pBuffer);
    }
        ...
        OMX_U32 nPortIndex)
{
    ...
    switch(sPortDef.eDomain) {
		...
		case OMX_AUDIO_CodingADPCM:
             	    {
			OMX_AUDIO_PARAM_ADPCMMODETYPE AdpcmPara;
			OMX_INIT_STRUCT(&AdpcmPara, OMX_AUDIO_PARAM_ADPCMMODETYPE);
			AdpcmPara.nPortIndex = nPortIndex;
			if(OMX_ErrorNone == OMX_GetParameter(hComponent, OMX_IndexParamAudioAdpcm, &AdpcmPara)) {
                            AdpcmPara.nPortIndex = nPeerPortIndex;
                            OMX_SetParameter(hPeer, OMX_IndexParamAudioAdpcm, &AdpcmPara);
                        }
		    break;
		    }
                ...
                case OMX_AUDIO_CodingOPUS:
                    {
                        OMX_AUDIO_PARAM_OPUSTYPE OpusPara;
                        OMX_INIT_STRUCT(&OpusPara, OMX_AUDIO_PARAM_OPUSTYPE);
                        OpusPara.nPortIndex = nPortIndex;
                        if(OMX_ErrorNone == OMX_GetParameter(hComponent, OMX_IndexParamAudioOpus, &OpusPara)) {
                            OpusPara.nPortIndex = nPeerPortIndex;
                            OMX_SetParameter(hPeer, OMX_IndexParamAudioOpus, &OpusPara);
                        }
                    }
                    break;
                case OMX_AUDIO_CodingAPE:
                    {
                        OMX_AUDIO_PARAM_APETYPE ApePara;
                        OMX_INIT_STRUCT(&ApePara, OMX_AUDIO_PARAM_APETYPE);
                        ApePara.nPortIndex = nPortIndex;
                        if(OMX_ErrorNone == OMX_GetParameter(hComponent, OMX_IndexParamAudioApe, &ApePara)) {
                            ApePara.nPortIndex = nPeerPortIndex;
                            OMX_SetParameter(hPeer, OMX_IndexParamAudioApe, &ApePara);
                        }
                    }
                    break;
                default:
...
OMX_ERRORTYPE GMComponent::LoadWithComponentHandle(OMX_HANDLETYPE handle, OMX_STRING role)
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;
    OMX_PARAM_PORTDEFINITIONTYPE sPortDef;
    OMX_U32 i;
    ...
    hComponent = handle;
    bIsSoftwareVideoDecoder = OMX_TRUE;
    fsl_osal_strcpy((fsl_osal_char*)name,(fsl_osal_char*)role);

    ((OMX_COMPONENTTYPE*)hComponent)->SetCallbacks(hComponent, &GMCallBacks, this);
    ret = SetGeneralSetting();
    for(i = 0; i < nPorts; i++) {
        // init this value to TRUE to force invoke DynamicPortSettingChange
        PortsInfo[i].bFisrtPortSettingChanged = OMX_FALSE;
    }
...
// extract from Load()
OMX_ERRORTYPE GMComponent::SetGeneralSetting()
{
    OMX_PARAM_PORTDEFINITIONTYPE sPortDef;
    OMX_U32 i;
    ...
    for(i=0; i<nPorts; i++) {
        ...
        if(E_FSL_OSAL_SUCCESS != fsl_osal_mutex_init(&PortsInfo[i].HoldBufferListLock, fsl_osal_mutex_normal)) {
            LOG_ERROR("Create mutex for HoldBufferList failed.\n");
        }
    }
    ...
    pBufAppData = NULL;
    EmptyDoneCb = NULL;
    EmptyDoneCbAppData = NULL;

    return OMX_ErrorNone;
}


OMX_ERRORTYPE GMComponent::GetBufferHdrOnPort(OMX_U32 nPortIndex, OMX_U32 index, OMX_BUFFERHEADERTYPE **ppBufferHdr)
{
    if (PortsInfo[nPortIndex].bEnabled != OMX_TRUE || index >= MAX_PORT_BUFFER) {
        *ppBufferHdr = NULL;
        return OMX_ErrorUndefined;
    }

    *ppBufferHdr = PortsInfo[nPortIndex].pBufferHdr[index];
