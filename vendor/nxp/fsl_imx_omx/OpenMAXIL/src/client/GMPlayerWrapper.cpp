...
static OMX_BOOL gm_setdivxdrmcallback(
        OMX_GraphManager* h __unused,
        void* context __unused,
        divxdrmcallback handler __unused,
        void *pfunc __unused)
{
    return OMX_TRUE;
}

static OMX_BOOL gm_preload(
        OMX_GraphManager *h,
        const char *filename,
        int length __unused)
{
    ...
    Player = (GMPlayer*)h->pData;
    if(OMX_ErrorNone != Player->PreLoad((OMX_STRING)filename))
        return OMX_FALSE;
...
static OMX_STRING gm_GetAudioTrackName(OMX_GraphManager* h, OMX_U32 nTrackIndex)
{
    OMX_U32 nAudioTrackNum;
    GMPlayer *Player = NULL;

    Player = (GMPlayer*)h->pData;
    return Player->GetAudioTrackName(nTrackIndex);
}
...
static OMX_U32 gm_GetSubtitleTrackNum(OMX_GraphManager* h)
{
    OMX_U32 nSubtitleTrackNum;
    GMPlayer *Player = NULL;
    Player = (GMPlayer*)h->pData;
    nSubtitleTrackNum = Player->GetSubtitleTrackNum();
    return nSubtitleTrackNum;
}

static OMX_U32 gm_GetSubtitleCurTrack(OMX_GraphManager* h)
{
    OMX_U32 nCurSubtitleTrack;
    GMPlayer *Player = NULL;
    Player = (GMPlayer*)h->pData;
    nCurSubtitleTrack = Player->GetSubtitleCurTrack();
    return nCurSubtitleTrack;
}

static OMX_STRING gm_GetSubtitleTrackName(OMX_GraphManager* h, OMX_U32 nTrackIndex)
{
    OMX_U32 nSubtitleTrackNum;
    GMPlayer *Player = NULL;
    Player = (GMPlayer*)h->pData;
    return Player->GetSubtitleTrackName(nTrackIndex);
}

static OMX_BOOL gm_SelectSubtitleTrack(OMX_GraphManager* h, OMX_U32 nSelectedTrack)
{
    OMX_BOOL ret = OMX_TRUE;
    GMPlayer *Player = NULL;
    Player = (GMPlayer*)h->pData;
    ret = Player->SelectSubtitleTrack(nSelectedTrack);
    return ret;
}

static OMX_U32 gm_GetSubtitleNextSample(OMX_GraphManager* h, OMX_U8**ppData, OMX_S32* pFilledLen, OMX_TICKS* pTime)
{
    GMPlayer *Player = NULL;
    OMX_ERRORTYPE ret;
    Player = (GMPlayer*)h->pData;
    ret=Player->GetSubtitleNextSample(ppData,pFilledLen,pTime,NULL);
    if(OMX_ErrorNoMore==ret){
        return 0;  //EOS
    }
    else if(OMX_ErrorNone!=ret){
        return 0;  //fail
    }
    else{
        return 1;
...
static OMX_BOOL gm_setVideoScalingMode(OMX_GraphManager *h,  OMX_S32 mode)
{
    GMPlayer *Player = NULL;

    Player = (GMPlayer*)h->pData;
    if(OMX_ErrorNone != Player->SetVideoScalingMode(mode))
        return OMX_FALSE;
...
static OMX_BOOL gm_setAudioPassThrough(OMX_GraphManager *h, OMX_BOOL bEnable)
{
    GMPlayer *Player = NULL;

    Player = (GMPlayer*)h->pData;
    if(OMX_ErrorNone != Player->SetAudioPassThrough(bEnable))
        return OMX_FALSE;

    return OMX_TRUE;
}

static OMX_BOOL gm_getAudioTrackType(OMX_GraphManager *h, OMX_U32 nTrackIndex, OMX_AUDIO_CODINGTYPE *pType)
{
    GMPlayer *Player = NULL;

    Player = (GMPlayer*)h->pData;
    if(OMX_ErrorNone != Player->GetAudioTrackType(nTrackIndex, pType))
        return OMX_FALSE;

    return OMX_TRUE;
}

static OMX_BOOL gm_addOutBandSubtitleSource(OMX_GraphManager *h, OMX_STRING url, OMX_STRING type)
{
    GMPlayer *Player = NULL;

    Player = (GMPlayer*)h->pData;
    if(OMX_ErrorNone != Player->AddOutBandSubtitleSource(url, type))
        return OMX_FALSE;

    return OMX_TRUE;
}

static OMX_BOOL gm_isSeekable(OMX_GraphManager *h)
{
    GMPlayer *Player = NULL;
...
OMX_GraphManager* OMX_GraphManagerCreate()
{
	...
	Gm->GetAudioTrackName = gm_GetAudioTrackName;
	Gm->GetCurAudioTrack = gm_GetCurAudioTrack;
	Gm->SelectAudioTrack = gm_SelectAudioTrack;
	Gm->GetSubtitleTrackNum = gm_GetSubtitleTrackNum;
	Gm->GetSubtitleCurTrack = gm_GetSubtitleCurTrack;
	Gm->GetSubtitleTrackName = gm_GetSubtitleTrackName;
	Gm->SelectSubtitleTrack = gm_SelectSubtitleTrack;
	Gm->GetSubtitleNextSample=gm_GetSubtitleNextSample;
    ...
    Gm->setVideoScalingMode = gm_setVideoScalingMode;
    Gm->setVideoRenderType = gm_setVideoRenderType;
    Gm->setAudioPassThrough = gm_setAudioPassThrough;
