...
ic_resize(OMX_ImageConvert *h,
          OMX_IMAGE_PORTDEFINITIONTYPE *in_format, OMX_CONFIG_RECTTYPE *pCropRect,
          OMX_U8 *in_y, OMX_U8 *in_u, OMX_U8 *in_v,
...
OMX_ImageConvert* OMX_ImageConvertCreate()
{
    ...
    ic->pColorConverter = cc16WrapperCreate();
...
static OMX_BOOL ic_delete(OMX_ImageConvert *h)
{
    ...
    cc16WrapperDelete(&h->pColorConverter);
          ...
          OMX_IMAGE_PORTDEFINITIONTYPE *out_format, OMX_U8 *out)
{
    ...
    if(!h->pColorConverter){
        LOG_ERROR("invalid converter!");
        return OMX_FALSE;
    }

    if(in_format->nStride < (OMX_S32)in_format->nFrameWidth)
        in_format->nStride = in_format->nFrameWidth;
    if(out_format->nStride < (OMX_S32)out_format->nFrameWidth)
        out_format->nStride = out_format->nFrameWidth;

    OMX_S32 ret = cc16WrapperInit(h->pColorConverter,
            ((in_format->nFrameWidth)&(~1)),
            ((in_format->nFrameHeight)&(~1)),
            ((in_format->nStride)&(~1)),
            pCropRect,
            in_format->eColorFormat,
            out_format->nFrameWidth,
            ((out_format->nFrameHeight)&(~1)),
            ((out_format->nStride)&(~1)),
            0);

    if(!ret){
        LOG_ERROR("init cc16 fail %d", ret);
        return OMX_FALSE;
    }

    ret = cc16WrapperSetMode(h->pColorConverter ,1);
    if(!ret){
        LOG_ERROR("set mode cc16 fail");
        return OMX_FALSE;
    }

    if(!in_u || !in_v)
        ret = cc16WrapperConvert(h->pColorConverter, (OMX_U8*)in_y, (OMX_U8*)out);
    else{
        OMX_U8* yuv_data[3] = {in_y, in_u, in_v};
        ret = cc16WrapperConvert2(h->pColorConverter, yuv_data, (OMX_U8*)out);
    }

    if(!ret){
        LOG_ERROR("convert fail");
        return OMX_FALSE;
    }
