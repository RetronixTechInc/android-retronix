...
#define FSL_NEW(type, params) new((fsl_osal_char*)__FILE__, __LINE__) type params
#define FSL_DELETE(ptr) { if(ptr) {delete(ptr); ptr=NULL;}}
#define FSL_DELETE_THIS(ptr) { if(ptr) {delete(ptr);}}
#define FSL_MALLOC(size) dbg_malloc(size, (fsl_osal_char*)__FILE__, __LINE__)
#define FSL_REALLOC(ptr, size) dbg_realloc(ptr, size, (fsl_osal_char*)__FILE__, __LINE__)
...
fsl_osal_void print_allnonfreememory();
