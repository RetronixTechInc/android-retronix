...
static int rotate_config(GRSurface *gr_draw)
{
    if (rotate_index<0)
    {
		//~ int orientation = android::base::GetIntProperty("ro.panel.orientation", 0);
    	int orientation = 0;
        ...
        else if (orientation == 90) rotate_index=1;
        else if (orientation == 180) rotate_index=2;
        else if (orientation == 270) rotate_index=3;
        else rotate_index=0;
        printf("[graphics] rotate_config %d orientation = %d\n", rotate_index, orientation);
    }
    return rotate_index;
}
