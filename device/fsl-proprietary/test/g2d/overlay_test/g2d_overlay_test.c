...
main(int argc, char **argv)
{
	...
	if(screen_get_context_property_iv(screen_ctx, SCREEN_PROPERTY_DISPLAY_COUNT, &count) < 0) {
	...
	free(screen_disps);
	if(screen_get_display_property_iv(screen_disp, SCREEN_PROPERTY_SIZE, dims) < 0) {
		printf("Unable to get display property\n");
		goto err0;
	}

	// Create a window we will use to write the bitmaps to. By default, the window is fullscreen.
	if(screen_create_window(&screen_bg_win, screen_ctx) < 0) {
		printf("Unable to create window\n");
	}
	vis = 0;
	if(screen_set_window_property_iv(screen_bg_win, SCREEN_PROPERTY_VISIBLE, &vis) < 0) {
		printf("Unable to set window property - visible\n");
		goto err2;
	}
	val = SCREEN_USAGE_READ | SCREEN_USAGE_WRITE;// | SCREEN_USAGE_NATIVE;
	if(screen_set_window_property_iv(screen_bg_win, SCREEN_PROPERTY_USAGE, &val) < 0) {
		printf("Unable to set window property - usage\n");
		goto err2;
	}

	val = SCREEN_FORMAT_RGBA8888;
	if(screen_set_window_property_iv(screen_bg_win, SCREEN_PROPERTY_FORMAT, &val) < 0) {
		printf("Unable to set window property - format\n");
		goto err2;
	}

	val = SCREEN_TRANSPARENCY_NONE;
	if (screen_set_window_property_iv(screen_bg_win, SCREEN_PROPERTY_TRANSPARENCY, &val)) {
		printf("Unable to set window property - transparency\n");
		goto err2;
	}

	if (screen_set_window_property_pv(screen_bg_win, SCREEN_PROPERTY_DISPLAY, (void **)&screen_disp) < 0) {
	...
	if(screen_get_window_property_pv(screen_bg_win, SCREEN_PROPERTY_RENDER_BUFFERS, (void **)&screen_buf) < 0) {
		printf("Unable to set buffer - render buffers: %s\n", strerror( errno ));
		goto err2;
	}
	val = 1;
	/*if(screen_set_buffer_property_iv(screen_buf, SCREEN_PROPERTY_PHYSICALLY_CONTIGUOUS, &val) < 0) {
		printf("Unable to set buffer property - contiguous: %s\n", strerror( errno ));
	}*/
	if(screen_get_buffer_property_llv(screen_buf, SCREEN_PROPERTY_PHYSICAL_ADDRESS, &buffer_phys) < 0) {
		printf("Unable to get buffer property - paddr: %s\n", strerror( errno ));
		goto err2;
	}
	g_fb0_phys = (int)buffer_phys;


	if (screen_get_buffer_property_pv(screen_buf, SCREEN_PROPERTY_POINTER, (void **)&ptr) < 0) {
	...
	g_fb0_size = screen_info.xres * screen_info.yres * screen_info.bits_per_pixel / 8;

	rect[2] = dims[0];
	rect[3] = dims[1];

	if(screen_post_window(screen_bg_win, screen_buf, 1, rect, SCREEN_WAIT_IDLE) < 0) {
