...
OMX_ERRORTYPE AudioParserBase::InstanceInit()
{
	...
	SetMetadata((OMX_STRING)"tracks", (OMX_STRING)"1", 1);
...
OMX_ERRORTYPE AudioParserBase::ParserID3Tag()
{
	...
	if (len > 0) {
		...
		{
		    OMX_U8 *pCopyBuffer = (OMX_U8 *)FSL_MALLOC(nReadLen);
            fsl_osal_memcpy(pCopyBuffer, pTmpBuffer, nReadLen);
			FSL_FREE(pTmpBuffer);
		...
		if (nReadLen + ID3V2_HEADER_SIZE > nFileSize)
		{
			nReadLen = nFileSize - ID3V2_HEADER_SIZE;
		}

		nActuralRead = fileOps.Read(sourceFileHandle, pTmpBuffer + ID3V2_HEADER_SIZE, nReadLen, this);

		OMX_BOOL bGetV2 = OMX_FALSE;

        isValid = ID3ParserParseV2(h, pTmpBuffer, nReadLen);
	...
	if (!isValid || 0 == len) {
		...
		fsl_osal_memset(pTmpBuffer, 0, AUDIO_PARSER_READ_SIZE);
		fileOps.Seek(sourceFileHandle, -ID3V1_DATA_SIZE, SEEK_END, this);
		nActuralRead = fileOps.Read(sourceFileHandle, pTmpBuffer, nReadLen, this);

		LOG_DEBUG("Parser ID3 TAG V1.\n");
        isValid = ID3ParserParseV1(h, pTmpBuffer, nReadLen);
    ...
    for (size_t i = 0; i < size; ++i) {
        SetMetadata((char *)meta[i].key, (char *)meta[i].value, strlen(meta[i].value));
        LOG_DEBUG("Key: %s  metadata: %s\n", (char *)meta[i].key, (char *)meta[i].value);
    }
