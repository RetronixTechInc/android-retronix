...
OMX_ERRORTYPE LibavAudioDec::AVConvertSample(OMX_U32 *len)
{
    ...
    OMX_U32 osize = av_get_bytes_per_sample(out_fmt);
    ...
    if (avr)
        ...
        if (!avr) {
            LOG_DEBUG("AVAudioResampleContext allocate failed!\n");
            return OMX_ErrorUndefined;
        }
    }

    data_size = av_samples_get_buffer_size(NULL, codecContext->channels, frame->nb_samples, \
    ...
    if (avresample_open(avr) < 0) {
        LOG_DEBUG("error initializing libavresample!\n");
        return OMX_ErrorUndefined;
    }

    out_size = av_samples_get_buffer_size(&out_linesize, codecContext->channels, nb_samples, out_fmt, 0);

    out_samples = avresample_convert(avr, &(pOutBufferHdr->pBuffer), out_linesize, nb_samples, \
                                          frame->data, frame->linesize[0], frame->nb_samples);
    if (out_samples < 0) {
        LOG_DEBUG("error convert libavresample!!!\n");
        return OMX_ErrorUndefined;
    }
...
AUDIO_FILTERRETURNTYPE LibavAudioDec::AudioFilterFrame()
{
    ...
    OMX_U8 *pBuffer = NULL;
    OMX_U32 nActuralLen = 0, nConsumeLen = 0, data_size = 0, nchannel = 0, nFrameLen = 0;
    int nOutSize = 0, len = 0;
    AVPacket pkt;
    AVCodec* codec;
    ...
    LOG_DEBUG("\t data_size: %d \t nConsumeLen: %d \t nFrameLen: %d\n", data_size, nConsumeLen, nFrameLen);
#if 0
    FILE * pfile;
